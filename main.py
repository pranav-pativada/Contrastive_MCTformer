...
def get_args_parser():
    ...
    parser.add_argument('--clr_weight', type=int, default=0)
...
def main(args):
    ...
    dataset_val, args.nb_classes = build_dataset(is_train=False, args=args, use_mIOU=True)
    ...
    if args.eval:
        ...
        print(f"mAP and best_mIOU of the network on the {len(dataset_val)}")
        print(f"test images mAP: {test_stats['mAP']*100:.1f}%")
        if "max_mIOU" in test_stats:
            print(f"test images best_mIOU: {test_stats['max_mIOU']*100:.1f}%")
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        best_mIOU = 0.0
        if args.mIOU_metric:
            best_mIOU = test_stats['best_mIOU']
            print(f"best mIOU of the network on the {len(dataset_val)} test images : {best_mIOU*100:.2f}%")
        if best_mIOU + test_stats['mAP'] > max_accuracy and args.output_dir:
        ...
        max_mIOU = max(max_mIOU, best_mIOU)
        print(f'Max mIOU: {max_mIOU * 100:.2f}%')
        max_accuracy = max(max_accuracy, best_mIOU + test_stats['mAP'])
